...
def runSingleScenario(scenarioFilename):
    ...
    try:
        ...
        logging.exception(
            "Failed running '%(scenarioFilename)s' as a test class", dict(scenarioFilename=scenarioFilename)
        )
        logging.shutdown()
        raise
    finally:
        logging.info("Done Running '%(scenarioFilename)s' as a test class", dict(scenarioFilename=scenarioFilename))
        logging.shutdown()
